function [Summary,Cluster,f] = GraphWeightSumPlus(A,varargin)
%
% Input: 
...
p.addParamValue('weights', repmat(1,k,k),@(x)ismatrix(x) && size(x,1)==k &&size(x,2)==k);
...
disp(e-t);
while flag
    [B,C] = WeightRefinement(B, C, A, r, powerA, res, hasCon,w);
    ...
    if isreduced
        [Summary,Cluster] = WeightRefinement(Summary, Cluster, A, r, powerA, res, hasCon,w);     
    end
